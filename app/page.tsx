...
const femaleStyles = ['Cute Tomboy Noona', 'Elegant Smart Noona', 'Bubbly Sexy Noona'];
  ...
  const lessons = [
    { id: 'lesson1', title: 'Basic Greetings', phrase: '안녕하세요, 저는 지민이에요', clipUrl: 'https://www.youtube.com/embed/I3Bmj1nCIko' },
    { id: 'lesson2', title: 'Asking for Directions', phrase: '여기 어떻게 가나요?', clipUrl: '/kdrama2.gif' }
  ]
...
async function generateAvatar() {
     ...
     console.log('saved avatarAdjectivesrompt', avatarAdjectives, gender, avatarName);
  try {
    // Step 1: Send a request to generate an image
    let url = "https://cloud.leonardo.ai/api/rest/v1/generations";
    const formattedAdjectives = avatarAdjectives.split(',').map(adj => adj.trim()).join(', ');
    ...
    <div className="min-h-screen bg-[#FFF5EA] p-4">
      ...
      {tab === 'setup' && (
        <div className="max-w-md mx-auto bg-white p-6 rounded-2xl shadow-lg border-2 border-[#E5E5E5]">
          <h2 className="text-2xl font-bold mb-6 text-[#4B4B4B]">Create Your AI Teacher! 🎮</h2>

          {/* Your Name input */}
          <label className="block mb-2 font-bold text-[#4B4B4B]">Your Name:</label>
          <input
          ...
          {/* Avatar Name input */}
          <label className="block mb-2 font-bold text-[#4B4B4B]">Avatar Name:</label>
          <input
            className="w-full border-2 border-[#E5E5E5] rounded-xl p-3 mb-4 focus:border-[#58CC02] focus:outline-none"
            value={avatarName}
          ...
          {/* Gender buttons */}
          <label className="block mb-2 font-bold text-[#4B4B4B]">Avatar Gender:</label>
          <div className="flex gap-4 mb-4">
            <button
              ...
              onClick={() => { setGender('male'); setAvatarStyle(''); }}
            >Male</button>
            <button
              className={`px-6 py-3 rounded-xl border-2 transition-all transform hover:scale-105 ${
                gender === 'female'
              ...
              onClick={() => { setGender('female'); setAvatarStyle(''); }}
            >Female</button>
          </div>

          {gender && (
            <>
              <label className="block mb-2 font-bold text-[#4B4B4B]">Choose a Style:</label>
              <select
                className="w-full border-2 border-[#E5E5E5] rounded-xl p-3 mb-4 focus:border-[#58CC02] focus:outline-none"
                value={avatarStyle}
              ...
              >
                ...
                {(gender === 'male' ? maleStyles : femaleStyles).map(style => (
                ...
                <>
                  <div className="mb-4">
                    <label className="block mb-2 font-bold text-[#4B4B4B]">
                      Describe your {gender === 'male' ? 'Oppa' : 'Noona'} (3 adjectives, comma separated)
                    </label>
                    <input
          ...
          <div className="mt-6">
            {(avatarStyle !== 'custom' && avatarStyle !== '' || (avatarImageUrl && avatarImageUrl.trim() !== '')) && (
              <>
      ...
      {tab === 'lessons' && !selectedLesson && (
        <div className="max-w-2xl mx-auto">
          <h2 className="text-2xl font-bold mb-6 text-[#4B4B4B] text-center">Pick a Lesson! 📚</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {lessons.map((lesson, index) => (
              <div
                key={lesson.id}
                className={`py-8 bg-white p-4 rounded-2xl shadow-lg border-2 border-[#E5E5E5] relative
                  ${index === 0 ? 'cursor-pointer transition-all transform hover:scale-105 hover:shadow-xl' :
                ...
                {index !== 0 && (
                  <div className="absolute inset-0 bg-black bg-opacity-10 rounded-2xl flex items-center justify-center">
                    <div className="bg-white/90 px-6 py-2 rounded-xl text-center">
                      ...
                      <p className="text-gray-600 font-medium mt-2">Complete Lesson 1 to unlock</p>
                    </div>
                  </div>
                )}
                <h3 className="font-bold text-lg text-[#4B4B4B]">{lesson.title}</h3>
              </div>
            ))}
          </div>
        </div>
      ...
      {tab === 'lessons' && selectedLesson && (
        <div className="max-w-2xl mx-auto bg-white p-6 rounded-2xl shadow-lg border-2 border-[#E5E5E5]">
          ...
          {lessons.filter(l => l.id === selectedLesson).map(lesson => (
<div key={lesson.id}>
  ...
  <div className="mb-4">
    ...
    <ul className="list-disc pl-5">
      <li>
        ...
        <p className="text-gray-500 italic">Translation: Hello.</p>
      </li>
      <li>
        <p className="text-lg font-bold">제 이름은 {userName || "ㅇㅇ"}입니다.</p>
        <p className="text-gray-400 text-sm">je ireumeun {userName || "OO"}-imnida</p>
        <p className="text-gray-500 italic">Translation: My name is {userName || "OO"}.</p>
      </li>
      <li>
        <p className="text-lg font-bold">반갑습니다.</p>
        <p className="text-gray-400 text-sm">bangapseumnida</p>
        <p className="text-gray-500 italic">Translation: Nice to meet you.</p>
      </li>
    </ul>
  </div>

  <div className="mt-4">
    <h3 className="font-semibold mb-2">Practice with {avatarName || `Your ${gender === 'male' ? 'Oppa' : 'Noona'} TA`}</h3>
